...
#define CONFIG_FACTORY_PARTITION_NAME "factory"
#define CONFIG_APP_WIFI_PROV_NAME_PREFIX
...
void set_wifi_psm_mode(void)
{
        ESP_LOGI(TAG, "esp_wifi_set_ps().");
        //esp_err_t err = esp_wifi_set_ps(DEFAULT_PS_MODE);
        esp_err_t err = esp_wifi_set_ps(DEFAULT_PS_MODE);
        if (err != ESP_OK)
        {
...
static esp_err_t read_random_bytes_from_nvs(uint8_t **random_bytes, size_t *len)
{
    ...
    *len = 0;
    ...
    if ((err = nvs_open_from_partition(CONFIG_FACTORY_PARTITION_NAME, CREDENTIALS_NAMESPACE,
                                NVS_READONLY, &handle)) != ESP_OK) {
        ESP_LOGD(TAG, "NVS open for %s %s %s failed with error %d", CONFIG_FACTORY_PARTITION_NAME, CREDENTIALS_NAMESPACE, RANDOM_NVS_KEY, err);
        return ESP_FAIL;
    }
...
static esp_err_t get_device_service_name(char *service_name, size_t max)
{
    ...
    const char *ssid_prefix = DEVICE_ID_PREFIX;
