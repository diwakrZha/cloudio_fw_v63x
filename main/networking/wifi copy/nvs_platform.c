...
bool nvs_slot_exit(esp_err_t err)
{
    ...
    case ESP_ERR_NVS_NOT_FOUND:
        return false;
    default:
        ESP_LOGI(TAG, "***** SEGMENTATION FAULT *****");
        break;
    }
    return false;
}
...
void nvs_perform_number(char *key, uint32_t *value, e_nvs_param_t type, bool read_write)
{
    ...
    char str_value[256]; // buffer for string operations, adjust size if necessary
    ...
    if (read_write)
    {
        ...
        case U32:
            ...
            break;
        case STR:
            err = nvs_set_str(my_handle, (const char *)key, (const char *)value); // Assuming value holds address to a string
            break;
        default:
            break;
        }
    ...
    else
    {
        ...
        case U32:
            ...
            break;
        case STR:
            err = nvs_get_str(my_handle, (const char *)key, str_value, &required_size);
            if (nvs_slot_exit(err))
            {
                // Convert the string value to your desired format if necessary and assign to *value
            }
            else
            {
                nvs_set_str(my_handle, (const char *)key, (const char *)value); // Assuming value holds address to a string
            }
            break;
        default:
            break;
...
void nvs_perform_str(char *key, char *value, e_nvs_param_t type, bool read_write)
{
    ...
    else
    {
        ...
        nvs_slot_exit(err) ? nvs_get_str(my_handle, (const char *)key, value, &required_size)
